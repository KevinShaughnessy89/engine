...
static std::string stripMaximoBullshit(const std::string& name) {
    auto pos = name.find(":");
...
inline std::vector<glm::mat4> boneOffsets;
inline const int MAX_BONES_PER_VERTEX = 4;
inline int numVertices;
inline std::vector<BoneInfo> skeleton;

inline glm::mat4 AssimpToGlm(const aiMatrix4x4 aiMatrix) {
    glm::mat4 glmMatrix;
...
inline void prepareNewMesh() {
    tempBoneData.clear();
}

inline void populateAccumulator(const aiMesh* mesh) {
...
inline void addBoneWeights(const aiMesh* mesh, std::vector<MeshVertex>& vertices) {
...
inline AnimationClip loadClip(const aiAnimation* anim) {
    ...
    for (unsigned int c = 0; c < anim->mNumChannels; c++) {
        ...
        std::cout << "  Channel '" << channel.boneName << "': " << nodeAnim->mNumPositionKeys
                  << " pos keys, " << nodeAnim->mNumRotationKeys << " rot keys, "
                  << nodeAnim->mNumScalingKeys << " scale keys" << std::endl;

        clip.channels[channel.boneName] = channel;
    }

    std::cout << "Loaded clip '" << clip.name << "': duration=" << clip.duration
              << "s ticksPerSecond=" << clip.ticksPerSecond << " channels=" << clip.channels.size()
              << std::endl;
